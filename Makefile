LIBS = -lSDL2 -lncurses -lm

CXXFLAGS = -march=native -fopenmp -mavx -D _DEBUG -ggdb3 -std=c++17 -Wall -Wextra \
           -Weffc++ -Waggressive-loop-optimizations -Wc++14-compat \
           ...
           -Wswitch-default -Wsync-nand -Wundef -Wunreachable-code -Wunused \
...
CXX = g++
SOURCES = Mandelbrot_simd.cpp Mandelbrot_main.cpp 
OBJECTS = $(SOURCES:.cpp=.o)

all: Mandelbrot

%.o: %.cpp
	$(CXX) -c $^ -o $@ $(CXXFLAGS) $(OPT)

Mandelbrot: $(OBJECTS)
	$(CXX) $(OBJECTS) -o $@ $(CXXFLAGS) $(OPT) $(LIBS)
